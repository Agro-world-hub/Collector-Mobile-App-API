...
exports.createCollectionOfficerSchema = Joi.object({
  ...
  phoneNumber1: Joi.string().required().trim().max(20).pattern(/^[0-9]+$/).messages({
    'string.empty': 'Phone number is required',
    'string.pattern.base': 'Phone number should contain only digits',
    'any.required': 'Phone number is required'
  }),
  phoneCode2: Joi.string().allow(null, '').trim().max(10),
  phoneNumber2: Joi.string().allow(null, '').trim().max(20).pattern(/^[0-9]+$/),
  ...
  // Profile image
  profileImage: Joi.string().allow(null, '').max(5000000) // Allow base64 strings with reasonable size limit
});
